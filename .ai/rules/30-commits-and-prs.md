...
- **Связанная задача** — `Closes PTR-XXX` (если PR полностью закрывает задачу) или `Refs PTR-XXX`. **По умолчанию один PR закрывает ровно одну задачу** — см. «Один issue = один PR» в [`50-task-management.md`](50-task-management.md).
